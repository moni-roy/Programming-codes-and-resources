...
ll tr[25][MX],laz[25][MX],a,ar[MX][25];

void kor(int p,int s,int e,int k)
{
	if(e==s)
	{
		tr[k][p]=ar[s][k];
	...
	kor(l,s,m,k);
	kor(r,m+1,e,k);
	tr[k][p]=tr[k][l]+tr[k][r];
}

void up(int p,int s,int e,int x,int y,int k)
{
	if(x==s && e==y)
	{
		tr[k][p]=e-s+1-tr[k][p];
	...
	if(y<=m) up(l,s,m,x,y,k);
	else if(x>m)up(r,m+1,e,x,y,k);
	else {
		up(l,s,m,x,m,k);
		up(r,m+1,e,m+1,y,k);
	}
...
ll dey(int p,int s,int e,int x,int y,int k){
	if(x==s && e==y) return tr[k][p];
	int m =(s+e)/2;
	int l = 2*p;
	int r=l+1;
	if(laz[k][p]){
	...
	if(y<=m) return dey(l,s,m,x,y,k);
	else if(x>m) return dey(r,m+1,e,x,y,k);
	return dey(l,s,m,x,m,k)+dey(r,m+1,e,m+1,y,k);
...
int main()
{
	...
	for(int i=0;i<22;i++) kor(1,1,n,i);
	cin>>m;
	while(m--){
		cin>>c;
		//~ CHK;
