...
int check(string a,string b,string c,int n)
{
    if(n<(int)a.size()) return 1;
    n-=(int)a.size();
    if(n<(int)b.size()) return 2;
    return 3;
}
...
int main()
{
	...
	while(ts--){
		...
		for(int i = 2; i <= l; i++)
		{
			int a = check(s1,s2,s3,Suffix[i].S);
			int b = check(s1,s2,s3,Suffix[i-1].S);
			int c = check(s1,s2,s3,Suffix[i-2].S);
			
			if(a==b || b==c || a==c) continue;
			
			int aa = lcp(Suffix[i].S,Suffix[i-1].S,l);
			int bb = lcp(Suffix[i-1].S,Suffix[i-2].S,l);
