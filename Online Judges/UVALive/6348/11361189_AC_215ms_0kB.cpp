...
int X1[MX],X2[MX],Y1[MX],Y2[MX],n;
struct st{
	int x,y,c;
	st(){}
	st(int _,int __,int ___){
		x = _;
		y = __;
		c = ___;
	}
...
set<pii>lw,up;
int Ans[MX];
int go(int y1,int y2){
	if(lw.empty()) return -1;
	auto it = lw.lower_bound({y1,-1});
	if(it ==lw.begin()) return -1;
	it--;
	int id = it->second;
	if(Y1[id]<y1 && Y2[id]>y2) return id;
	it = up.lower_bound({y1,-1});
	if(it == up.begin()) return -1;
	it--;
...
int main()
{
	...
	for(int i = 0;i<n;i++){
		...
		ar[i*2] = st(X1[i],i,1);
		ar[i*2+1] = st(X2[i],i,0);
	}
	sort(ar,ar+2*n);
	for(int i = 0;i<2*n;i++){
		int id = ar[i].y;
		if(ar[i].c){
			Ans[id] = go(Y1[id],Y2[id]);
			lw.insert({Y1[id],id});
			up.insert({Y2[id],id});
		}
		else{
